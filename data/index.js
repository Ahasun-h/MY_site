...
function home() {
    $.getJSON("data/home.json", function async (data) {
        ...
        document.getElementById("home_completed_project").innerHTML =
            data[0].data.complete_projects;
        document.getElementById("home_satisfied_user").innerHTML =
            data[0].data.satisfied_customer;
    ...
    $.getJSON("data/home.json", function async (data) {
        ...
        for (let i = 0; i < data[1].service.service_item.length; i++) {
            $("#service_item").append(
                ...
                '<h5 class="vlt-services__title">\n' + data[1].service.service_item[i].name + '</h5>\n' +
                '<p class="vlt-services__text">\n' + data[1].service.service_item[i].service_description + '</p>\n' +
                "</div>\n" +
                "</div>\n" +
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[2].education.length; i++) {
            $("#education_items").append(
                ...
                '<p class="vlt-timeline-item__title">' + data[2].education[i].certification + '</p>\n' +
                '<span class="vlt-timeline-item__year">' + data[2].education[i].session + '</span>\n' +
                '<h5 class="vlt-timeline-item__text" style="font-weight: normal;color: #fff;">' + data[2].education[i].institute_name + '</h5>\n' +
                "</div>\n" +
                "</div>"
            );
        }
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[3].professional_experience.length; i++) {
            $("#professional_experience").append(
                ...
                '</div>\n' +
                '<div class="resume-text">\n' +
                '<p>' + data[3].professional_experience[i].company_name + '</p>\n' +
                '<span>' + data[3].professional_experience[i].work_experience + '</span>\n' +
                '<h5>' + data[3].professional_experience[i].position + '</h5>\n' +
                '</div>\n'+
                '</div>'
            );
        }
...
function  trainingExperience() {
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[4].training_experience.length; i++) {
            $("#training_experience").append('<div class="resume-step">\n' +
                ' <div class="resume-status">\n' +
                '   <span class="resume-status-dot"></span>\n' +
                '   <span class="resume-status-line"></span>\n' +
                ' </div>\n' +
                ' <div class="resume-text">\n' +
                '   <p>'+data[4].training_experience[i].training_name+'</p>\n' +
                '   <span>'+data[4].training_experience[i].training_period+'</span>\n' +
                '   <h5>'+data[4].training_experience[i].training_institute+'</h5>\n' +
                ' </div>\n' +
                '</div>');
        }
    }).fail(function () {
...
function aboutMe() {
    $.getJSON("data/home.json", function async (data) {
        document.getElementById("my_name").innerHTML =
            data[5].about_me.name;
        document.getElementById("about_me").innerHTML =
            data[5].about_me.my_self;
        document.getElementById("dob").innerHTML =
            data[5].about_me.date_of_birth;
        document.getElementById("gender").innerHTML =
            data[5].about_me.gender;
        document.getElementById("mobile_number").innerHTML =
            data[5].about_me.mobile;
        document.getElementById("email").innerHTML =
            data[5].about_me.email;
        document.getElementById("address").innerHTML =
            data[5].about_me.address;
        document
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[6].portfolio.portfolio_menu.length; i++) {
            $("#portfolio_menu").append(
                '<li class="control" data-filter="'+ data[6].portfolio.portfolio_menu[i].data_filter+'">\n' +
                '<a href="#" class="mix_itup_menu" >'+data[6].portfolio.portfolio_menu[i].menu+'</a>\n' +
                '</li>\n'+
                '<li>/</li>'
            );
        }
        ...
        for (let i = 0; i < data[6].portfolio.portfolio_item.length; i++) {
            $("#portfolio_item").append(
                '<div class="portfolio_item col-12 col-sm-6 col-md-4 col-lg-4 col-xl-4 mix '+data[6].portfolio.portfolio_item[i].portfolio_type+'" data-ref="mixitup-target">\n' +
                '  <img class="images"  id="portfolio_image" src="https://ahasun180.000webhostapp.com/assets/img/loding.gif" data-src="'+data[6].portfolio.portfolio_item[i].portfolio_image+'">\n' +
                '  <div class="stacks" style=""><div class="row position-relative m-0">'+loadStacks(data[6].portfolio.portfolio_item[i])+'</div></div>\n' +
                '  <div class="portfolio_item_shadow">\n' +
                '        <a href="'+data[6].portfolio.portfolio_item[i].portfolio_link+'">\n' +
        ...
        var $portfolio_images = $("#portfolio_item").find("img");
        var loaded_images = 0;
        $portfolio_images.on("load", function () {
            loaded_images++;
            if (loaded_images === $portfolio_images.length) {
                $portfolio_images.unveil();
            }
        });
    }).fail(function () {
        console.log("Data Error");
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[7].testimonial.testimonial_item.length; i++) {
            $("#testimonial_item_append").append(
                ...
                '       <img id="testimonial_image" src="'+data[7].testimonial.testimonial_item[i].image+'" alt="'+data[7].testimonial.testimonial_item[i].name+'">\n' +
                '   </div>\n' +
                '   <div class="vlt-testimonial-item__content">\n' +
                '       <p id="testimonial_content">'+data[7].testimonial.testimonial_item[i].content+'</p>\n' +
                '       <div class="vlt-testimonial-item__meta">\n' +
                '           <h6 id="testimonial_name">'+data[7].testimonial.testimonial_item[i].name+'</h6>\n' +
                '           <span id="testimonial_position">'+data[7].testimonial.testimonial_item[i].position+'</span>\n' +
                '       </div>\n' +
                '   </div>\n' +
                '</div>\n'+
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[7].testimonial.work_with.length; i++) {
            $("#work_with").append(
                ...
                '         <img src="'+data[7].testimonial.work_with[i].image+'" alt="'+data[7].testimonial.work_with[i].name+'" style="height: auto;width: 100%;object-fit: cover;">\n' +
                '     </div>\n' +
                '  </div>\n' +
    ...
    $.getJSON("data/home.json", function async (data) {
        for (let i = 0; i < data[9].social_link.length; i++) {
            $(".vlt-fixed-socials").append(
                '<a href="'+data[9].social_link[i].link+'" target="_blank">\n' +
                '  <i class="'+data[9].social_link[i].icon+'"></i>\n' +
                '</a>'
            );
        }
    }).fail(function () {
